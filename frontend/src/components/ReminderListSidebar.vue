...
<template>
  <aside class="reminder-list-sidebar" aria-label="Reminder lists">
    <section class="sidebar-section" aria-label="Smart lists">
      <p class="sidebar-heading">Smart Lists</p>

      <ul class="list-navigation">
        <li
          v-for="smartList in smartLists"
          :key="smartList.key"
          class="list-navigation-item smart-list-navigation-item"
        >
          <button
            class="list-select-button"
            type="button"
            :class="{
              'is-selected': selectedView.type === 'smart' && selectedView.key === smartList.key
            }"
            :aria-current="
              selectedView.type === 'smart' && selectedView.key === smartList.key
                ? 'page'
                : undefined
            "
            @click="selectSmartList(smartList.key)"
          >
            <span class="list-select-name">{{ smartList.name }}</span>
            <span class="list-count">{{ smartList.count }}</span>
          </button>
        </li>
      </ul>
    </section>

    <section class="sidebar-section" aria-label="My lists">
      <div class="sidebar-header">
        <p class="sidebar-heading">My Lists</p>
        <button
        ...
        <li v-for="list in lists" :key="list.id" class="list-navigation-item">
          ...
          <template v-else>
            <button
              ...
              type="button"
              :class="{
                'is-selected': selectedView.type === 'list' && list.id === selectedView.id
              }"
              :aria-current="
                selectedView.type === 'list' && list.id === selectedView.id ? 'page' : undefined
              "
              @click="selectList(list.id)"
            >
              <span class="list-select-name">{{ list.name }}</span>
            </button>
